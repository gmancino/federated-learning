...
w_true = np.concatenate((w_true, np.array([b_true])), axis=0)
...
for i in range(1, 21):
    ...
    corruption_log_reg = MulticlassLogReg(test_data[0:num_test], y_corrupt, np.random.rand(test_data.shape[1], 10), np.random.rand(10), T=1000, tol=0.5, alpha=0.25, lam1=1e-3, lam2=0.1, num_classes=10)
